...
function preserveInstructionReference(value: unknown): string {
...
function runtimeGateMatchesCommitScope(
  runtime: MarrowAgentRuntimeResult,
  params: { action: string; decision_id: string },
  sessionId?: string,
  agentId?: string,
): boolean {
...
function isDurableObservedOutcome(value: Record<string, unknown>): boolean {
...
type AgentWriteKind = 'think' | 'commit';
...
async function waitForWriteReconciliation(signal?: AbortSignal): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    const abort = () => {
      ...
      reject(normalizeRequestError(signal?.reason || new DOMException('Aborted', 'AbortError')));
    };
    if (signal?.aborted) {
      abort();
      return;
...
export async function marrowThink(
  ...
  const { response, data } = await fetchAgentWrite(
    thinkUrl,
    thinkInit,
    'think',
    idempotencyKey,
...
export async function marrowCommit(
  ...
  if (data.outcome_state === 'observed_unverified') {
    ...
    return { ...data, committed: false, runtime_gate: runtimeGate } as CommitResult & {
      runtime_gate?: MarrowAgentRuntimeResult | null;
    };
  }
  if (observationOnly) throw invalidResponseError();
  if (response.status === 202
    && params[AUTO_MANAGED_WRITE] === true
    && (data.phase === undefined || data.phase === 'commit_pending' || data.resumable === true)) {
    return markAutoResponseStatus(
      { ...data, committed: false, runtime_gate: runtimeGate } as CommitResult,
      response.status,
    );
  }
  ...
  return markAutoResponseStatus(
    { ...data, committed: data.committed, runtime_gate: runtimeGate } as CommitResult,
    response.status,
  );
}
...
async function marrowAutoWithTrace(
  ...
  if (runtimeGate && !runtimeGate.arbitration && (runtimeGate.decision_id || ordinaryApprovalDeclared)) {
    ...
    }) || (params.gate_receipt_id && params.gate_receipt_id !== gateReceiptId)) throw invalidResponseError();
  ...
  let pendingThinkDecisionId: string | null = operationBinding.pendingThinkDecisionId || null;
  while (!decisionId) {
    ...
    if (!isAutoPendingResponse(thinkResult, 'think_pending')) {
      if ((thinkResult as AutoPendingResponse)[AUTO_RESPONSE_STATUS] === 202 || !receivedDecisionId) {
        throw invalidResponseError();
      }
  ...
  const arbitrationRequiresOwnerApproval = Boolean(
    runtimeGate?.arbitration
  ...
  if (!proofCanClose) {
    ...
    const ownerApprovedCommitAttempt = Boolean(
      arbitrationRequiresOwnerApproval
      && params.proof
