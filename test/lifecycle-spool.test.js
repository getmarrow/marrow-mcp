...
test('native MCP hook receipts carry bounded capability and actual configuration evidence', async () => {
  ...
  writeFileSync(join(settingsDir, 'settings.json'), JSON.stringify({
    hooks: {
      UserPromptSubmit: [{ hooks: [{ type: 'command', command: 'npx -y @getmarrow/mcp@3.9.61 context-hook' }] }],
      PreToolUse: [{ matcher: 'Bash|Edit|Write|MultiEdit|mcp__(?!marrow__marrow_).*', hooks: [{ type: 'command', command: 'npx -y @getmarrow/mcp@3.9.61 pre-action-hook' }] }],
      PostToolUse: [{ matcher: 'Bash|Edit|Write|MultiEdit|mcp__(?!marrow__marrow_).*', hooks: [{ type: 'command', command: 'npx -y @getmarrow/mcp@3.9.61 hook' }] }],
      PostToolUseFailure: [{ matcher: 'Bash|Edit|Write|MultiEdit|mcp__(?!marrow__marrow_).*', hooks: [{ type: 'command', command: 'npx -y @getmarrow/mcp@3.9.61 hook' }] }],
      Stop: [{ hooks: [{ type: 'command', command: 'npx -y @getmarrow/mcp@3.9.61 session-hook' }] }],
    },
  }));
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async () => new Response('{}', { status: 503 });
  try {
    await withSpoolPath(path, async () => {
      ...
      assert.equal(event.adapter_version, '3.9.61');
