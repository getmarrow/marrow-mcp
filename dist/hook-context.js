...
function compareClientVersions(left, right) {
    ...
    if (!parsedLeft || !parsedRight)
        throw new Error('compareClientVersions requires strict semantic versions');
...
function boundedClientCommand(value) {
    if (typeof value !== 'string' || value.length > 240 || /[\u0000-\u001f\u007f-\u009f\u2028\u2029]/.test(value))
        return undefined;
    return value.trim() || undefined;
}
function validClientCommandTarget(value) {
...
function appendAgentRuntime(lines, runtime) {
    ...
    if (habit && (habit.interrupt === true || firstHour)) {
        if (asString(habit.headline))
            lines.push(`- Habit: ${asString(habit.headline)}`);
        ...
        if (asString(habit.exact_next_action))
            lines.push(`- First-hour next: ${asString(habit.exact_next_action)}`);
    }
...
function compactRuntimeContext(runtime) {
...
async function runContextHookCommand() {
    ...
    try {
        ...
        let context = live.value
            ? passiveBriefInput
                ? compactRuntimeContext(live.value)
                : compactAgentContext(live.value)
            : '';
        if (context) {
            try {
                (0, guidance_cache_1.writeGuidanceCache)({ apiKey, baseUrl, agentId, context });
            }
            ...
            let cached = null;
            try {
                cached = (0, guidance_cache_1.readGuidanceCache)({ apiKey, baseUrl, agentId });
            }
            catch { /* cache is best effort */ }
            if (cached) {
