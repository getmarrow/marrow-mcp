...
function resolveHostCapability(input = {}) {
    ...
    const reported = [...new Set(input.clientSelfReportedActivity || [])]
        .filter((activity) => typeof activity === 'string' && KNOWN_CLIENT_ACTIVITY.has(activity));
    const has = (activity) => reported.includes(activity);
    const completeNativeHookActivity = exports.NATIVE_HOOK_ACTIVITY.every(has);
    ...
    return {
        ...
        passive_hooks: {
            ...
            client_activity_stages: [...exports.NATIVE_HOOK_ACTIVITY],
        },
        capability_modes: {
