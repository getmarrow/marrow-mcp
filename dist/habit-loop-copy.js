...
exports.modelUsageCaptureContextFromEnv = modelUsageCaptureContextFromEnv;
...
function modelUsageCaptureContextFromEnv() {
    let pricing_dimensions;
    const raw = process.env.MARROW_MODEL_USAGE_PRICING_DIMENSIONS;
    if (raw && raw.length <= 2048) {
        try {
            pricing_dimensions = (0, model_usage_1.normalizeModelUsage)({ pricing_dimensions: JSON.parse(raw) }).pricing_dimensions;
        }
        catch { /* Unknown configuration remains unpriced. */ }
    }
...
function extractModelUsageFromUnknown(source, context = {}) {
    const root = asRecord(source);
    if (!root)
        return null;
    ...
    if (!usage)
        return null;
    const number = (value) => typeof value === 'number' && Number.isSafeInteger(value) && value >= 0 ? value : undefined;
    const numberAt = (...keys) => {
    ...
    if (numericKeys.some(key => Object.hasOwn(usage, key) && number(usage[key]) === undefined))
        return null;
    for (const detail of [asRecord(usage.input_tokens_details), asRecord(usage.prompt_tokens_details)]) {
        if (detail && ['cached_tokens', 'cache_write_tokens'].some(key => Object.hasOwn(detail, key) && number(detail[key]) === undefined))
            return null;
    }
    ...
    try {
        ...
        if (endpoint.protocol === 'https:' && !endpoint.username && !endpoint.password && !endpoint.port) {
            endpointProvider = { 'api.openai.com': 'openai', 'api.anthropic.com': 'anthropic' }[endpoint.hostname];
        }
    }
    ...
    if ([input_tokens, output_tokens, cached_tokens, cache_write_tokens, total_tokens].every(value => value === undefined))
        return null;
    ...
    if (tier && (openai || anthropic))
        dims.tier = tier === 'default' && openai ? 'standard' : tier;
    ...
    if (anthropic && creation) {
        ...
        else if (cache_write_tokens)
            dims.cache_ttl = 'unresolved';
    }
    ...
    try {
        return (0, model_usage_1.normalizeModelUsage)({ provider, model, input_tokens, output_tokens, cached_tokens, cache_write_tokens, total_tokens,
