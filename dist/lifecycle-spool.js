...
function reservedSpoolBytes(events) {
...
function validateStoredEvent(value) {
    ...
    if (event.last_status != null && (!Number.isInteger(event.last_status)
        || Number(event.last_status) < 0 || Number(event.last_status) > 599))
        throw new Error('invalid lifecycle last_status');
    if (event.retry_reason != null && !RETRY_REASONS.has(event.retry_reason))
        throw new Error('invalid lifecycle retry_reason');
    if (event.retry_blocked != null && typeof event.retry_blocked !== 'boolean')
        throw new Error('invalid lifecycle retry_blocked');
    if (event.recovery_attempts != null && (!Number.isInteger(event.recovery_attempts)
        || Number(event.recovery_attempts) < 0))
        throw new Error('invalid lifecycle recovery_attempts');
    if (event.recovery_exhausted != null && typeof event.recovery_exhausted !== 'boolean')
        throw new Error('invalid lifecycle recovery_exhausted');
    if (event.server_owned != null && typeof event.server_owned !== 'boolean')
        throw new Error('invalid lifecycle server_owned');
    ...
    const stored = {
        ...
        ...(RETRY_REASONS.has(event.retry_reason) ? { retry_reason: event.retry_reason } : {}),
        ...
        ...(event.recovery_exhausted === true ? { recovery_exhausted: true } : {}),
        ...(event.server_owned === true ? { server_owned: true } : {}),
    };
...
function deadLetterClass(event) {
    ...
    if (event.server_owned === true || event.last_status === 409)
        return 'server_owned';
    if (event.last_status === 401 || event.last_status === 403)
        return 'auth';
    return 'recoverable';
}
function retryAfter(response) {
    ...
    if (!Number.isFinite(milliseconds) || milliseconds > MAX_SERVER_RETRY_MS)
        return { retryBlocked: true };
...
function lifecycleSpoolStatus(input) {
    ...
    for (const event of current.events) {
        if (event.retry_reason)
            retryReasons[event.retry_reason] = (retryReasons[event.retry_reason] || 0) + 1;
    }
    return {
...
async function attemptQueuedDelivery(input) {
    ...
    mutate(input.path, input.ownsParent, (events) => {
        ...
        if (!retryable(status)) {
            ...
            if (status === 409)
                current.server_owned = true;
            if ((current.recovery_attempts ?? 0) >= RECOVERY_MAX_ATTEMPTS)
                current.recovery_exhausted = true;
        }
            ...
            current.next_attempt_at = new Date(Math.max(existingMinimum, Date.now() + Math.max(backoffMs, result.retryAfterMs || 0))).toISOString();
        }
    });
    return status;
}
...
async function recoverLifecycleDeadLetters(input, deadline) {
    const location = spoolPath(input.apiKey, input.agentId);
    ...
    const recovered = mutate(location.path, location.ownsParent, (events) => {
        ...
        for (const row of events) {
            ...
            if (classification === 'server_owned') {
                ...
                if (row.server_owned !== true)
                    row.server_owned = true;
                continue;
            }
            if (classification === 'auth')
                continue;
            if (row.recovery_exhausted === true)
                continue;
            ...
            if (row.last_recovery_at && now - Date.parse(row.last_recovery_at) < RECOVERY_COOLDOWN_MS)
                continue;
            if (selected.length >= RECOVERY_MAX_EVENTS_PER_NUDGE)
                continue;
    ...
    if (recovered.length === 0 || remainingMs <= 0)
        return;
...
async function drainLifecycleSpool(input) {
    ...
    while (delivered < maxEvents && Date.now() < deliveryDeadline) {
        const candidates = snapshot(location.path, location.ownsParent).events.filter((row) => (row.delivery_state === 'queued' && (input.retryWithinBudget || !attempted.has(row.event_id))));
        ...
        if (!queued) {
            ...
            if (!Number.isFinite(earliest) || earliest >= deliveryDeadline)
                break;
...
async function recordLifecycleEvent(input) {
    ...
    if (deliveryStatus >= 200 && deliveryStatus < 300) {
        const previous = snapshot(location.path, location.ownsParent).events.find((row) => (row.delivery_state === 'queued' && row.event_id !== event.event_id && dueAt(row) <= Date.now()));
        if (previous) {
            try {
