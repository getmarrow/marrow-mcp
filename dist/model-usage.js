...
exports.MODEL_USAGE_EVIDENCE_PROPERTIES = void 0;
exports.normalizeModelUsage = normalizeModelUsage;
const redact_1 = require("./redact");
const stringFields = ['agent_id', 'session_id', 'workflow_id', 'decision_id', 'provider', 'model', 'task_type', 'action_type', 'source', 'marrow_intervention', 'billing_host', 'usage_event_id', 'baseline_usage_id', 'comparison_id', 'task_fingerprint', 'constraints_fingerprint'];
const numberFields = ['input_tokens', 'output_tokens', 'cached_tokens', 'cache_write_tokens', 'total_tokens', 'cost_usd', 'latency_ms', 'baseline_tokens', 'estimated_tokens_saved', 'estimated_cost_saved_usd', 'estimated_minutes_saved'];
...
const booleanFields = ['success', 'coverage_complete', 'overhead_complete'];
...
function normalizeModelUsage(input = {}) {
    if (!input || typeof input !== 'object' || Array.isArray(input))
        throw new TypeError('model_usage must be an object');
    const body = {};
    const invalid = (key) => { throw new TypeError(`Invalid model usage ${key}`); };
    for (const key of stringFields) {
        const value = input[key];
        if (value === undefined || (value === null && ['agent_id', 'session_id', 'workflow_id', 'decision_id'].includes(key)))
            continue;
    ...
    for (const key of numberFields) {
        ...
        if (value === undefined)
            continue;
        if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || (key.endsWith('tokens') && !Number.isSafeInteger(value)))
            invalid(key);
        body[key] = value;
    }
    ...
    if (input.occurred_at !== undefined) {
        if (typeof input.occurred_at !== 'string' || input.occurred_at.length > 40 || !/^\d{4}-\d{2}-\d{2}T/.test(input.occurred_at) || !Number.isFinite(Date.parse(input.occurred_at)))
            invalid('occurred_at');
    ...
    if (input.pricing_dimensions !== undefined) {
        ...
        if (!dims || typeof dims !== 'object' || Array.isArray(dims) || Object.keys(dims).length > 16)
...
exports.MODEL_USAGE_EVIDENCE_PROPERTIES = {
