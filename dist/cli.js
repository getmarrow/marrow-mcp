...
function parseArgs() {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (['hook', '--hook', 'claude-hook', 'codex-hook', 'cursor-hook', 'grok-hook'].includes(args[i])) {
            result.hook = true;
        }
        ...
        if (['pre-action-hook', '--pre-action-hook', 'claude-pre-action-hook', 'codex-pre-action-hook', 'cursor-pre-action-hook', 'grok-pre-action-hook'].includes(args[i])) {
            result.preActionHook = true;
        }
        if (['session-hook', '--session-hook', 'claude-session-hook', 'codex-session-hook', 'cursor-session-hook', 'grok-session-hook'].includes(args[i])) {
