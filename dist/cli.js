...
function parseArgs() {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (['hook', '--hook', 'claude-hook', 'cline-hook', 'codex-hook', 'cursor-hook', 'grok-hook'].includes(args[i])) {
            result.hook = true;
        }
        ...
        if (['pre-action-hook', '--pre-action-hook', 'claude-pre-action-hook', 'cline-pre-action-hook', 'codex-pre-action-hook', 'cursor-pre-action-hook', 'grok-pre-action-hook'].includes(args[i])) {
            result.preActionHook = true;
        }
        if (['session-hook', '--session-hook', 'claude-session-hook', 'cline-session-hook', 'codex-session-hook', 'cursor-session-hook', 'grok-session-hook'].includes(args[i])) {
...
if (process.argv[2] !== 'keys') {
    if (cliArgs.hook) {
        ...
        const TOOLS = [
            ...
            {
                name: 'marrow_run',
                description: 'Governed outcome capture. Records intent, obtains a gate for risky work, and closes only with an explicit measured result.',
                inputSchema: {
                    type: 'object',
                    properties: {
            ...
            {
                name: 'marrow_auto',
                description: 'Durably capture low-risk activity. One outer invocation normally completes in-band within the bounded client budget; a deadline continuation reuses the same operation_id and never opens a second decision. Outcomes remain pending unless success is explicit; risky completion still requires a fresh gate and measured proof.',
                inputSchema: {
                    type: 'object',
                    properties: {
